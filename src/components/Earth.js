...
export class Earth extends PureComponent {
  componentDidMount() {
    ...
    const geometry = new this.THREE.BoxGeometry(5, 5, 5);
    ...
    //LIGHTS
    var lights = [];
    lights[0] = new this.THREE.PointLight(0x304ffe, 1, 0);
    lights[1] = new this.THREE.PointLight(0xffffff, 1, 0);
    lights[2] = new this.THREE.PointLight(0xffffff, 1, 0);
  ...
  onLoad = texture => {
    var objGeometry = new this.THREE.SphereBufferGeometry(3, 35, 35);
    var objMaterial = new this.THREE.MeshPhongMaterial({
      map: texture,
      shading: this.THREE.FlatShading
    });

    this.earthMesh = new this.THREE.Mesh(objGeometry, objMaterial);
